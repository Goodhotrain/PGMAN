...
class Cformer(nn.Module):
    def __init__(self,
        ...
        # classif_head
        self.head_v = nn.Linear(2048,self.visual_embed_size)
        self.head = classif_head(self.visual_embed_size + self.visual_embed_size, n_classes, drop=0.5)

        self.n=torch.nn.Sigmoid()
        self.head2 = classif_head(self.visual_embed_size, n_classes, drop=0.3)
        freeze(self.head2,self.cross)
    ...
    def forward(self, visual: torch.Tensor, audio: list, text: list):
        ...
        with torch.no_grad():
            ...
            visual = visual.contiguous()
            visual_embedded = self.resnet(x).squeeze()
            visual_embedded = self.head_v(visual_embedded)
            visual_embedded = rearrange(visual_embedded, '(b t) c -> b t c',b=b,t=8)
        ...
        if self.need_audio:
            ...
            for a_p in audio:
                with torch.no_grad():
                    a_F= self.auformer(a_p, False)  # [B x 256]
        ...
        output = torch.cat([F_V, F_A], dim=1)
        output = torch.max(self.n( self.head(output)),self.n(self.head2(out_align)))
