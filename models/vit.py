...
from tools.model import load_state_dict
from models.vit_utils import DropPath, to_2tuple, trunc_normal_
from tools.model import freeze, unfreeze
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        freeze(self.drop_path, self.attn, self.norm1, self.norm2, self.mlp)
...
class TubeletEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=(2,16,16), stride=(2,16,16))

    def forward(self, x):
        B, C, T, H, W = x.shape
        x = rearrange(x, 'b c t h w -> b t c h w')
        x = rearrange(x, 'b (t h1) c h w -> (b t) c h1 h w', h1 =2)
        x = self.proj(x)
        W = x.size(-1)
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x)
        W = x.size(-1)
        x = x.flatten(2).transpose(1, 2) # b*t,  m*n, 768
        return x, T, W
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        B = x.shape[0]
        x, T, W = self.patch_embed(x) # b*t, 196, 768
        cls_tokens = self.cls_token.expand(x.size(0), -1, -1) # b*t, 1, 768
        x = torch.cat((cls_tokens, x), dim=1) # b*t, 1+196, 768
        ...
        if x.size(1) != self.pos_embed.size(1):
            ...
            x = x + self.pos_embed
        x = self.pos_drop(x) # torch.Size([8*8, 196, 768])


        ## Time Embeddings
        if self.attention_type != 'space_only':
            cls_tokens = x[:B, 0, :].unsqueeze(1)
            x = x[:,1:] # torch.Size([8*8, 196, 768])
            x = rearrange(x, '(b t) n m -> (b n) t m',b=B,t=T) # torch.Size([8*196, 8, 768])
            ...
            if T != self.time_embed.size(1):
                ...
                x = x + self.time_embed
            x = self.time_drop(x) # torch.Size([8*196, 8, 768])
            x = rearrange(x, '(b n) t m -> b (n t) m',b=B,t=T) # torch.Size([8, 196*8, 768])
            x = torch.cat((cls_tokens, x), dim=1)

        ## Attention blocks
        for blk in self.blocks:
            x = blk(x, B, T, W)
        # torch.Size([8, 1569, 768])
...
class Vit(nn.Module):
    def __init__(self, img_size=224, patch_size=16, num_classes=400, num_frames=16, attention_type='divided_space_time',  pretrained_model='', **kwargs):
        ...
        self.spatial_former = vit_b_16(weights='DEFAULT')
        self.spatial_former.heads = torch.nn.Identity()
        self.attention_type = attention_type
        self.model.default_cfg = default_cfgs['vit_base_patch'+str(patch_size)+'_224']
        self.num_patches = (img_size // patch_size) * (img_size // patch_size)
        if pretrained_model:
            state_dict = load_state_dict(pretrained_model)
            filtered_state_dict = {k: v for k, v in state_dict.items() if k in self.model.state_dict() and v.size() == self.model.state_dict()[k].size()}
            self.model.load_state_dict(filtered_state_dict, strict=False)
    
    # @torch.no_grad()
    def forward(self, x, t):
        y = self.model(x)
        return  y[:, 0], y ,self.spatial_former(t)